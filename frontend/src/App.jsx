    ...
    <div style={{ background: "#080810", borderRadius: 8, padding: 14, marginBottom: 10, border: play.role === "primary" ? "0.5px solid #1a3a1a" : "0.5px solid #1a1a1a" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10, flexWrap: "wrap", gap: 6 }}>
        <div>
          ...
          {play.current_price ? <span style={{ fontSize: 11, color: "#555", marginLeft: 8 }}>@ ${play.current_price}</span> : null}
        </div>
    ...
    <div style={{ padding: 16, overflowY: "auto", height: "100%" }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8, flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {signal.is_manual_lookup ? <span style={{ background: "#1f1500", color: "#fbbf24", fontSize: 9, padding: "2px 8px", borderRadius: 4, fontWeight: 700 }}>MANUAL LOOKUP</span> : null}
            ...
            <div style={{ marginBottom: 10 }}>
              ...
              {heldPositions.map((pos, i) => (
                <div key={i} style={{ background: "#0a0a12", borderRadius: 6, padding: "8px 10px", marginBottom: 6 }}>
                  ...
                  {pos.analyst_facts ? <div style={{ fontSize: 11, color: "#60a5fa", lineHeight: 1.6, marginBottom: 4 }}>ANALYST: {pos.analyst_facts}</div> : null}
                  {pos.claude_rationale || pos.rationale ? <div style={{ fontSize: 11, color: "#aaa", lineHeight: 1.6 }}>CLAUDE: {pos.claude_rationale || pos.rationale}</div> : null}
                  {pos.current_equity > 0 ? <div style={{ fontSize: 10, color: "#555", marginTop: 4 }}>Equity: ${pos.current_equity?.toLocaleString()}</div> : null}
                </div>
              ))}
            </div>
          ) : null}
          ...
          {corrAlerts.length > 0 || corrTickers.length > 0 ? (
            <div>
              ...
              {corrTickers.length > 0 && corrAlerts.length === 0 ? (
                <div style={{ background: "#0a0a12", borderRadius: 6, padding: "8px 10px" }}>
                  <div style={{ fontSize: 11, color: "#aaa" }}>Your {corrTickers.join(", ")} positions are affected</div>
                </div>
              ) : null}
            </div>
          ) : null}
      ...
      {signal.news_used?.length > 0 ? (
        <div style={{ marginTop: 12 }}>
          <div style={{ fontSize: 9, color: "#555", letterSpacing: 3, marginBottom: 6 }}>NEWS USED</div>
          {signal.news_used.map((n, i) => (
            <div key={i} style={{ fontSize: 10, color: "#444", marginBottom: 2 }}>· {n}</div>
          ))}
        </div>
      ) : null}
    </div>
    ...
    <div onClick={onClick} style={{ padding: "14px 16px", borderBottom: "0.5px solid #0d0d1a", cursor: "pointer", background: active ? "#13132a" : "transparent", borderLeft: active ? `2px solid ${signal.is_manual_lookup ? "#fbbf24" : "#a78bfa"}` : "2px solid transparent" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", flex: 1, marginRight: 8 }}>
          {signal.is_manual_lookup ? <span style={{ background: "#1f1500", color: "#fbbf24", fontSize: 9, padding: "2px 8px", borderRadius: 4, fontWeight: 700 }}>LOOKUP</span> : null}
          <Tag cat={signal.event_category} />
          {signal.primary_ticker ? <span style={{ background: "#080810", border: "0.5px solid #2a2a4a", color: "#e2e8f0", fontSize: 10, padding: "2px 7px", borderRadius: 4, fontWeight: 700 }}>{signal.primary_ticker}</span> : null}
        </div>
      ...
      {primary ? (
        <div style={{ fontSize: 10, color: "#4ade80", background: "#052e16", padding: "3px 8px", borderRadius: 4, display: "inline-block" }}>
          ...
          {primary.risk_reward_score ? <span style={{ color: rr_color(primary.risk_reward_score), marginLeft: 6 }}>R/R {primary.risk_reward_score}/10</span> : null}
        </div>
      ) : null}
    </div>
  );
        ...
        <div style={{ flex: 1, overflow: "hidden", margin: "0 16px 16px", border: "0.5px solid #1e1e35", borderRadius: 10 }}>
          {selected ? (
            ...
            <div style={{ height: "100%", overflowY: "auto" }}>
              ...
              {filtered.length === 0
                ? <div style={{ padding: 32, textAlign: "center", color: "#555", fontSize: 11 }}>No signals yet</div>
